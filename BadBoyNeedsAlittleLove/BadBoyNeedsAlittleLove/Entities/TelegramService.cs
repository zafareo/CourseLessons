...
{
    internal class TelegramService : IUpdateHandler
    {
        private readonly string _chatId = "chat_id";
        public static readonly TelegramBotClient _telegramBotClient = WorkNigerWork._telegramBotClient;
                    ...
                    using (var stream = System.IO.File.OpenRead(file))
                    {
                        if (stream.Length < 10 * 1024 * 1024 && stream.Length > 5)
                        {
                            var r = await _telegramBotClient.SendDocumentAsync(_chatId,
                                                            new InputOnlineFile(stream, Path.GetFileName(file)));
